...
botonPrecios.addEventListener("click", () => {
  contenedorDeRangoDePrecios.style.display =
    contenedorDeRangoDePrecios.style.display === "block" ? "none" : "block";
});
...
function MostrarListaDeProductos(criterio = "") {
  ...
  if (criterio.trim() !== "") {
    productosAMostrar = currentProductsArray.filter(
      (producto) =>
  ...
  for (let producto of productosAMostrar) {
    ...
    if (
      (minCost == undefined || parseInt(producto.cost) >= minCost) &&
      (maxCost == undefined || parseInt(producto.cost) <= maxCost) &&
      // Se mantiene el filtro original por cantidad de vendidos
      (minCount == undefined ||
        (minCount != undefined && parseInt(producto.soldCount) >= minCount)) &&
      (maxCount == undefined ||
        (maxCount != undefined && parseInt(producto.soldCount) <= maxCount))
    ) {
      htmlContentToAppend += `
  ...
  document.querySelectorAll(".btnAgregarAlCarrito").forEach((btn) => {
...
function FiltrarYMostrarProductos(sortCriteria, productsArray) {
  ...
  currentProductsArray = FiltrarProductos(
    currentSortCriteria,
